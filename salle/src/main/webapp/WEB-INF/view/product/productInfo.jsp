	...
	<%-- <img>는 컨텐츠일 때 background-image와 중첩으로 쓰인다. 중요하기 때문 --%>
		...
		<%--<button id="imgButton" onclick="imgPopup()">버튼</button>  --%>
			<c:forEach var="img" items="${productInfoImg}" varStatus="loop">
					<div class='div_pr_img' id='pr_img_${loop.index}' style='background-image: url("${s3Url}${img}")'>
						<input type="button" value="" id="img" class="button_img" name="${s3Url}${img}" onclick="imgPopup(this.name)"/>
					</div>
			</c:forEach>
